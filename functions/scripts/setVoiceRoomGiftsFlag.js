...
async function main() {
  ...
  const reason = readArgument('--reason') || (enabled ? 'Enable room gifts for testing' : 'Disable room gifts');
  const requestId = readArgument('--request-id')
    || `room_gifts_flag_${Date.now().toString(36)}_${Math.random().toString(36).slice(2, 14)}`;
  ...
  const patch = { voice_room_gifts: enabled };
  ...
  const auditRef = db.doc(`adminAuditEvents/voice_room_gifts_${requestId}`);
  await db.runTransaction(async (transaction) => {
    ...
    if (auditSnapshot.exists) {
      ...
      if (
        previous.action === 'voice-room-gifts-flag-update'
        && previous.actorUid === actorUid
        && previous.after?.voice_room_gifts === enabled
      ) return;
      throw new Error('The request ID conflicts with an existing audit event.');
    }
    const before = { voice_room_gifts: configSnapshot.data()?.voice_room_gifts === true };
    const after = { voice_room_gifts: enabled };
    ...
    transaction.create(auditRef, {
      action: 'voice-room-gifts-flag-update',
