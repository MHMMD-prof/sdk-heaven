...
async function main() {
  ...
  const reason = readArgument('--reason')
    || (enabled ? 'Enable Super Moderator operations' : 'Disable Super Moderator operations');
  const requestId = readArgument('--request-id')
    || `room_super_mod_flag_${Date.now().toString(36)}_${Math.random().toString(36).slice(2, 14)}`;
  ...
  const patch = { voice_room_super_moderation: enabled };
  ...
  const auditRef = db.doc(`adminAuditEvents/voice_room_super_moderation_${requestId}`);
  await db.runTransaction(async (transaction) => {
    ...
    if (auditSnapshot.exists) {
      ...
      if (
        previous.action === 'voice-room-super-moderation-flag-update'
        && previous.actorUid === actorUid
        && previous.after === enabled
      ) return;
      throw new Error('The request ID conflicts with an existing audit event.');
    }
    const before = configSnapshot.data()?.voice_room_super_moderation === true;
    ...
    transaction.create(auditRef, {
      action: 'voice-room-super-moderation-flag-update',
      actorEmail: actor.email || '',
      actorUid,
      after: enabled,
      before,
      createdAt: timestamp,
      flag: 'voice_room_super_moderation',
      kind: 'settings',
      note: reason,
      requestId,
