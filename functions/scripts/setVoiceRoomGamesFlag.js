...
async function main() {
  ...
  const reason = readArgument('--reason')
    || (enabled ? 'Enable voice room games' : 'Disable voice room games');
  const requestId = readArgument('--request-id')
    || `room_games_flag_${Date.now().toString(36)}_${Math.random().toString(36).slice(2, 14)}`;
  ...
  const patch = { voice_room_games: enabled };
  ...
  const auditRef = db.doc(`adminAuditEvents/voice_room_games_${requestId}`);
  await db.runTransaction(async (transaction) => {
    ...
    if (auditSnapshot.exists) {
      ...
      if (
        previous.action === 'voice-room-games-flag-update'
    ...
    const before = configSnapshot.data()?.voice_room_games === true;
    ...
    transaction.create(auditRef, {
      action: 'voice-room-games-flag-update',
      actorEmail: actor.email || '',
      actorUid,
      after: enabled,
      before,
      createdAt: timestamp,
      flag: 'voice_room_games',
      kind: 'settings',
      note: reason,
      requestId,
