...
export default function StoreCatalogEditor({ editor, onClose, onSaved, user }: { editor: CatalogEditorState; onClose: () => void; onSaved: () => void; user: User }) {
  ...
  return <Drawer title={existing ? 'تعديل عنصر الكتالوج' : editor.duplicate ? 'نسخ عنصر الكتالوج' : 'إضافة عنصر جديد'} subtitle="المعاينة والبيانات التجارية" onClose={() => void requestClose()}><form className="economy-editor-form" onSubmit={submit}><PreviewCard item={draft} previewFile={preview} thumbnailFile={thumbnail} />{errors.length ? <div className="economy-form-errors" role="alert"><strong>راجع البيانات قبل الحفظ</strong><ul>{errors.map((error) => <li key={error}>{error}</li>)}</ul></div> : null}<div className="economy-form-grid">
    ...
    {draft.category === 'cars' ? <EntryPresentationEditorFields approval={entryPhysicalApproval} itemNameAr={draft.name.ar} onApprovalChange={setEntryPhysicalApproval} onChange={(entryPresentation) => setDraft({ ...draft, entryPresentation })} value={draft.entryPresentation || defaultEntryPresentation} /> : null}
    {draft.category === 'couple-effects' ? <CoupleEffectEditorFields draft={draft} onChange={setDraft} /> : null}
    {draft.category === 'stickers' ? <><label className="field"><span>معرّف أصل الملصق</span><input dir="ltr" pattern="[a-z0-9][a-z0-9_-]{2,79}" required value={draft.stickerAsset?.assetId || ''} onChange={(e) => setDraft({ ...draft, stickerAsset: { assetId: e.target.value, assetVersionId: draft.stickerAsset?.assetVersionId || '' } })} /></label><label className="field"><span>إصدار الملصق غير القابل للتغيير</span><input dir="ltr" pattern="v[1-9][0-9]{0,8}-[a-f0-9]{12}" required value={draft.stickerAsset?.assetVersionId || ''} onChange={(e) => setDraft({ ...draft, stickerAsset: { assetId: draft.stickerAsset?.assetId || '', assetVersionId: e.target.value } })} /></label></> : null}
...
function CoupleEffectEditorFields({ draft, onChange }: { draft: StoreCatalogDraft; onChange: (draft: StoreCatalogDraft) => void }) {
  ...
  return <fieldset className="span-2"><legend>العرض المشترك المعتمد</legend><div className="economy-form-grid">
    <label>معرّف أصل couple-effect<input pattern="[a-z0-9][a-z0-9_-]{2,79}" required value={draft.cosmeticAsset?.assetId || ''} onChange={(event) => onChange({ ...draft, cosmeticAsset: { assetId: event.target.value, assetVersionId: draft.cosmeticAsset?.assetVersionId || '' } })} /></label>
    <label>الإصدار غير القابل للتغيير<input pattern="v[1-9][0-9]{0,8}-[a-f0-9]{12}" required value={draft.cosmeticAsset?.assetVersionId || ''} onChange={(event) => onChange({ ...draft, cosmeticAsset: { assetId: draft.cosmeticAsset?.assetId || '', assetVersionId: event.target.value } })} /></label>
