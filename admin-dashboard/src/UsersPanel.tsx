...
function UserWorkspace({ nextUid, onChanged, onClose, onNavigate, onSectionChange, permissions, previousUid, section, targetUid, user }: { nextUid: string; onChanged: () => Promise<void>; onClose: () => void; onNavigate: (uid: string) => void; onSectionChange: (section: UserWorkspaceSection) => void; permissions: string[]; previousUid: string; section: UserWorkspaceSection; targetUid: string; user: User }) {
  ...
  const { canAddNotes, canManageEconomy, canManageUsers } = getUserWorkspaceCapabilities(permissions);
  ...
  return <div className="user-drawer-backdrop" onMouseDown={(event) => { if (event.currentTarget === event.target) onClose(); }}><aside aria-label="ملف المستخدم" aria-modal="true" className="user-drawer" role="dialog">
    ...
    {state.status === 'ready' ? <div className="user-dossier-body">
      ...
      <main className="user-dossier-main"><nav className="user-drawer-tabs" aria-label="أقسام ملف المستخدم">{userWorkspaceSections.map((item) => <button className={section === item ? 'selected' : ''} key={item} onClick={() => onSectionChange(item)} type="button">{sectionLabel(item)}</button>)}</nav><div className="user-tab-content">
        {section === 'overview' ? <><UserDataWarning message={state.detail.context.errors.notifications} /><OverviewSection detail={state.detail} onCopy={copyValue} /></> : null}
...
function RepresentativePermissions({ canManage, detail, onChanged, user }: { canManage: boolean; detail: AdminUserDetail; onChanged: () => Promise<void>; user: User }) {
  ...
  const [busy, setBusy] = useState(false);
  async function save(event: FormEvent) { event.preventDefault(); if (!canManage || (active && !coins && !diamonds)) return; const approved = await confirm({ confirmLabel: 'حفظ الصلاحيات', description: active ? `اعتماد صلاحية ${coins && diamonds ? 'العملات والماس' : coins ? 'العملات' : 'الماس'}.` : 'إيقاف صلاحية الوكيل فوراً.', destructive: !active, title: 'تأكيد صلاحيات الوكيل؟' }); if (!approved) return; setBusy(true); try { await updateAdminRepresentative(user, { active, coins, diamonds, expectedUpdatedAt: detail.representative.updatedAt, targetUid: detail.profile.uid }); notify('تم تحديث صلاحيات الوكيل', { tone: 'success' }); await onChanged(); } catch (error) { notify('تعذّر تحديث صلاحيات الوكيل', { description: error instanceof Error ? error.message : undefined, tone: 'error' }); } finally { setBusy(false); } }
  return <form className="wallet-adjustment-form" onSubmit={save}><div className="user-card-heading"><div><p className="eyebrow">صلاحية تشغيلية</p><h4>الوكيل وإعادة الشحن</h4></div><AdminStatusBadge tone={detail.representative.active ? 'success' : 'neutral'}>{detail.representative.active ? 'نشط' : 'غير نشط'}</AdminStatusBadge></div>{canManage ? <><label><input checked={active} onChange={(event) => setActive(event.target.checked)} type="checkbox" /> وكيل نشط</label><label><input checked={coins} onChange={(event) => setCoins(event.target.checked)} type="checkbox" /> إرسال العملات</label><label><input checked={diamonds} onChange={(event) => setDiamonds(event.target.checked)} type="checkbox" /> إرسال الماس</label><button disabled={busy || (active && !coins && !diamonds)} type="submit">حفظ صلاحيات الوكيل</button></> : <ReadOnlyNotice />}</form>;
}
...
function sectionLabel(section: UserWorkspaceSection) { return ({ overview: 'نظرة عامة', moderation: 'السلامة', economy: 'الاقتصاد', social: 'العلاقات', notes: 'الملاحظات', activity: 'النشاط' } as Record<UserWorkspaceSection, string>)[section]; }
