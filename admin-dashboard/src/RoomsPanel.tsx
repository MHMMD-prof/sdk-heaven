...
function RoomDrawer({ onChanged, onClose, roomId, user }: { onChanged: () => Promise<void>; onClose: () => void; roomId: string; user: User }) {
  ...
  async function runAction(roomAction: AdminRoomAction, target?: AdminRoomMember) {
    ...
    const destructive = ['close-room', 'remove-member', 'transfer-host'].includes(roomAction);
...
function roomActionLabel(action: AdminRoomAction | string) { return ({ 'close-room': 'إغلاق الغرفة', 'mute-member': 'كتم المشارك', 'remove-member': 'إزالة المشارك', 'reopen-room': 'إعادة فتح الغرفة', 'transfer-host': 'نقل الاستضافة', 'unmute-member': 'رفع كتم المشارك' } as Record<string, string>)[action] || action || 'إجراء إشرافي'; }
