...
export function DirectChatScreen({ navigation, route }: DirectChatScreenProps) {
  const { user } = useAuth();
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      void writeDirectChatDraft(user.uid, thread.conversationId, draft);
    }, 350);
  ...
  const send = async () => {
    ...
    const sent = await thread.send(text, reply?.id || '');
    if (sent) {
      setDraft('');
      setReply(undefined);
      thread.setTyping(false);
      typingActive.current = false;
      if (user?.uid && thread.conversationId) await clearDirectChatDraft(user.uid, thread.conversationId);
    }
  };
  ...
  return (
    <LinearGradient colors={['#020202', '#100506', '#020202']} style={styles.root}>
      <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : undefined} style={styles.root}>
        ...
        {requestIncoming ? (
          <RequestPanel
            ...
            onReport={() => Alert.alert('الإبلاغ', 'سيتم تفعيل التقاط أدلة البلاغ الآمنة في موجة الإشراف المخصصة.')}
          />
        ) : null}
...
function MessageBubble({ bubble, cosmetics, isMine, message, messages, onLongPress, peerReadSequence, uid }: { bubble?: EquipmentCosmeticProjection; cosmetics: CosmeticsFeatureFlags; isMine: boolean; message: DirectChatUiMessage; messages: DirectChatUiMessage[]; onLongPress: () => void; peerReadSequence: number; uid: string }) {
  ...
  if (system) return <Text style={styles.systemMessage}>{message.text || systemLabel(message.systemType)}</Text>;
  return (
    <Pressable
    ...
    >
      ...
      {message.text || message.visibilityState === 'unsent' ? <Text maxFontSizeMultiplier={1.45} selectable style={[styles.messageText, message.visibilityState === 'unsent' && styles.unsentText]}>
        {message.visibilityState === 'unsent' ? t('تم إلغاء إرسال هذه الرسالة', 'This message was unsent') : message.text}
      </Text> : null}
...
function EmptyThread({ requestOutgoing }: { requestOutgoing: boolean }) {
  return <View style={styles.emptyThread}><SymbolView name={{ ios: 'lock.shield.fill', android: 'lock', web: 'lock' }} size={34} tintColor={colors.gold} /><Text style={styles.emptyTitle}>{requestOutgoing ? t('تم إرسال طلب المحادثة', 'Chat request sent') : t('ابدأ حديثاً جديداً', 'Start a new conversation')}</Text><Text style={styles.emptyBody}>{t('المحادثة مرئية للمشاركين فقط، لكنها ليست مشفرة من طرف إلى طرف.', 'This chat is visible only to its participants, but it is not end-to-end encrypted.')}</Text></View>;
}
...
function showMessageOptions({ isMine, message, onReply, onRetry, onUnsend }: { isMine: boolean; message: DirectChatUiMessage; onReply: () => void; onRetry: () => void; onUnsend: () => void }) {
  const actions = [
...
const styles = StyleSheet.create({
  bubble: { borderRadius: 18, gap: spacing.xs, marginVertical: 3, maxWidth: '82%', minWidth: 92, overflow: 'hidden', paddingHorizontal: spacing.md, paddingVertical: spacing.sm },
  bubbleCosmetic: { bottom: 0, left: 0, opacity: 0.48, position: 'absolute', right: 0, top: 0 },
  center: { alignItems: 'center', flex: 1, justifyContent: 'center' },
  composerArea: { backgroundColor: '#080304', borderTopColor: colors.borderGold, borderTopWidth: 1, paddingHorizontal: spacing.md, paddingTop: spacing.sm },
  ...
  delivery: { color: colors.goldSoft, fontSize: 9 },
  ...
  emptyTitle: { color: colors.goldSoft, fontSize: 18, fontWeight: typography.weights.black },
  errorBanner: { backgroundColor: 'rgba(184,41,75,0.2)', padding: spacing.sm },
  ...
  header: { alignItems: 'center', backgroundColor: '#080304', borderBottomColor: colors.borderGold, borderBottomWidth: 1, flexDirection: 'row', gap: spacing.sm, minHeight: 76, paddingBottom: spacing.sm, paddingHorizontal: spacing.md },
  headerCopy: { flex: 1 },
  ...
  input: { backgroundColor: '#16090A', borderColor: colors.borderGold, borderRadius: 22, borderWidth: 1, color: colors.text, flex: 1, fontSize: 15, maxHeight: 120, minHeight: 46, paddingHorizontal: spacing.md, paddingVertical: spacing.sm, textAlign: TEXT_ALIGN, writingDirection: WRITING_DIRECTION },
  loadOlder: { alignSelf: 'center', backgroundColor: colors.surface, borderRadius: radius.full, marginBottom: spacing.md, paddingHorizontal: spacing.lg, paddingVertical: spacing.sm },
  loadOlderText: { color: colors.goldSoft, fontSize: 12 },
  ...
  messageTime: { color: colors.textSubtle, fontSize: 9 },
  mine: { alignSelf: 'flex-end', backgroundColor: '#65131C', borderBottomRightRadius: 5 },
  nameLine: { alignItems: 'center', flexDirection: ROW_DIRECTION, gap: spacing.xs },
  presence: { color: colors.textMuted, fontSize: 11, textAlign: TEXT_ALIGN },
  pressed: { transform: [{ scale: 0.97 }] },
  replyComposer: { alignItems: 'center', backgroundColor: 'rgba(232,190,97,0.08)', borderRightColor: colors.gold, borderRightWidth: 3, flexDirection: 'row', gap: spacing.sm, marginBottom: spacing.sm, padding: spacing.sm },
  ...
  requestAction: { alignItems: 'center', borderColor: colors.borderGold, borderRadius: radius.full, borderWidth: 1, minWidth: 64, paddingHorizontal: spacing.md, paddingVertical: 7 },
  ...
  requestPanel: { backgroundColor: '#17090A', borderBottomColor: colors.borderGold, borderBottomWidth: 1, padding: spacing.md },
  requestPrimary: { backgroundColor: colors.gold },
  ...
  root: { flex: 1 },
  roundButton: { alignItems: 'center', backgroundColor: colors.surface, borderColor: colors.borderGold, borderRadius: radius.full, borderWidth: 1, height: 42, justifyContent: 'center', width: 42 },
  sendButton: { alignItems: 'center', backgroundColor: colors.gold, borderRadius: radius.full, height: 46, justifyContent: 'center', width: 46 },
  systemMessage: { alignSelf: 'center', color: colors.textMuted, fontSize: 11, marginVertical: spacing.sm, textAlign: 'center' },
  theirs: { alignSelf: 'flex-start', backgroundColor: '#221719', borderBottomLeftRadius: 5 },
