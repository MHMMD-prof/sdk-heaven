...
import { encodeBattleshipLiveKitPayload, decodeBattleshipLiveKitPayload } from './liveKitBattleshipPayload';
import { requestBattleshipLiveKitConnectOptions } from './requestBattleshipLiveKitConnectOptions';
import {
...
export class LiveKitBattleshipTransport implements BattleshipTransport {
  constructor(
    private readonly roomFactory: LiveKitRoomFactory = () => new Room() as unknown as LiveKitRoomLike,
    private readonly requestConnectOptions = requestBattleshipLiveKitConnectOptions,
  ) {}
  ...
  async connect(options: BattleshipConnectOptions): Promise<BattleshipConnection> {
    ...
    const connection = new LiveKitBattleshipConnection(room, options.playerId);
...
class LiveKitBattleshipConnection implements BattleshipConnection {
  ...
  constructor(private readonly room: LiveKitRoomLike, localPlayerId: string) {
    ...
    this.dataReceivedListener = ((payload: Uint8Array, participant?: LiveKitParticipantLike) => {
      const message = decodeBattleshipLiveKitPayload(payload, Date.now());
  ...
  async publish(message: BattleshipOutboundMessage): Promise<void> {
    await this.room.localParticipant.publishData(encodeBattleshipLiveKitPayload(message), {
      reliable: isReliableBattleshipTopic(message.topic),
      topic: message.topic,
    });
  }
  ...
  private emitMessage(message: BattleshipInboundMessage) {
  ...
  private getPresence(): BattleshipPresencePlayer[] {
    return [
...
function mapLiveKitPresenceParticipant(participant: LiveKitParticipantLike): BattleshipPresencePlayer {
  return {
...
const isReliableBattleshipTopic = (_topic: BattleshipOutboundMessage['topic']) => true;

export const __testing = {
  isReliableBattleshipTopic,
